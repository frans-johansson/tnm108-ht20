...
# Shuffle the rows of the data, using the same random_state
# X = shuffle(boston.data, random_state=0)
# Y = shuffle(boston.target, random_state=0)
X = boston.data
Y = boston.target
cv = 10
...
# We get a better result after the shuffle!
...
# Results are worse compared to shuffled data and non-shuffled data,
# even though it probably should be better!
...
method = 'random forest trees'
clf = RandomForestClassifier(random_state=0)
y_pred = cross_val_predict(clf, X,Y, cv=cv)
CalcMeasures(method,y_pred,Y)

method = 'radial support vector machine'
...
# %% Print occurance count of each label in the data set
