...
# %% Using sklearn's Gaussian NB classifier
...
# %% Our own Gaussian NB classifier implementation
...
class GaussianNB:
    ...
    def group_by_class(self, data, target):
        ...
        # [x * 3 for x in list1 if x < 3] --> [3, 6]
        separated = [
            [x for x, t in zip(data, target) if t == c] for c in self.target_values
        ]
    ...
    def predict(self, data):
        ...
        """
        predicted_targets = [
            self.get_map(row) for row in data
        ]
        # for row in data:
...
model = GaussianNB()
model.target_values = target_values
model.train(X_train, y_train)

# Test the accuracy
print(
    f"""
    Using {X_train.shape[0]} rows for training and {X_test.shape[0]} for testing
    Accuracy: {model.accuracy(y_test, model.predict(X_test))}
    """
)
